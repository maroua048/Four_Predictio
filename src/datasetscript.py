...
# 2. Génération des paramètres (on garde les mêmes plages que l'image OCP)
...
# Cs Fuel (Conso spécifique) (9 - 12)
cs_fuel = 10 + 1.5 * np.sin(np.arange(hours) / 200) + np.random.normal(0, 0.5, hours)
cs_fuel = np.clip(cs_fuel, 9, 12)
...
df = pd.DataFrame({
    ...
    'Cs_Fuel': cs_fuel,                        # Conso spécifique (CIBLE)
...
# 4. Sauvegarde
df.to_csv('donnees_four_ocp_2026.csv', index=False)
print(f"✅ Dataset généré avec succès ! {len(df)} lignes (Fév → Août 2026).")
print("📁 Fichier : 'donnees_four_ocp_2026.csv'")
